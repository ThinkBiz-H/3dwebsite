...
const editorsSelection = computed(() =>
  featuredPosts.value.filter((p) => p.id !== featured.value?.id),
);
...
useSeoMeta(() => ({
  title: "Blog",
  description:
    "Plain-language lessons and updates on blockchain, wallets, and digital assets from the Lumen Ledger team.",
  path: "/blog",
  jsonLd: {
...
<template>
  ...
  <div class="space-y-24 py-24 sm:space-y-28">
    <template v-if="!loading && !isFiltering">
      <FeaturedCarousel
        v-if="editorsSelection.length"
        :posts="editorsSelection"
        base-path="/blog"
      />
      <TrendingRow v-if="trending.length" :posts="trending" base-path="/blog" />
      <EditorsChoiceSection
        v-if="editorsChoice.length"
        :posts="editorsChoice"
        base-path="/blog"
      />
      <MostPopularSection
        v-if="trending.length > 2"
        :posts="trending.slice(0, 3)"
        base-path="/blog"
      />
      <RecentlyUpdatedSection
        v-if="recentlyUpdated.length"
      ...
      <ExploreByTopic
        :posts="posts"
        base-path="/blog"
        @select-category="jumpToCategory"
      />
    </template>

    <section
      ref="filterBarEl"
      class="relative mx-auto max-w-7xl scroll-mt-24 px-6 lg:px-10"
    >
      <Breadcrumbs :items="[{ label: 'Blog' }]" class="mb-8" />

      <div class="border-t border-slate-200 pt-10">
        <p
          class="text-xs font-semibold uppercase tracking-[0.25em] text-blue-600"
        >
          Every lesson
        </p>
        <h2
          class="mt-2 font-display text-2xl font-bold tracking-tight text-gray-900 sm:text-3xl"
        >
          Latest articles
        </h2>
      </div>
      ...
      <div class="mt-10 grid grid-cols-1 gap-10 lg:grid-cols-[1fr,300px]">
        <div>
          ...
          <EmptyState
            ...
            :title="
              isFiltering ? 'No posts match your filters.' : 'No posts yet.'
            "
            :message="
              isFiltering
