...
const api = computed(() =>
  props.postType === 'blog'
    ? { getById: getBlogById, slugExists: blogSlugExists, create: createBlog, update: updateBlog }
    : { getById: getArticleById, slugExists: articleSlugExists, create: createArticle, update: updateArticle }
)
const listRouteName = computed(() => (props.postType === 'blog' ? 'admin-blogs' : 'admin-articles'))
...
async function persist(payload) {
  if (docId.value) {
    ...
    router.replace({
      name: props.postType === 'blog' ? 'admin-blog-edit' : 'admin-article-edit',
  ...
  () => {
    ...
    autoSaveTimer = setTimeout(async () => {
      ...
      try {
        ...
        console.error('[post-editor] auto-save failed:', err.code, err.message, err)
        autoSaveState.value = ''
      }
    }, 2000)
  }
...
<template>
  <div class="mx-auto max-w-5xl">
    <div class="flex flex-wrap items-center justify-between gap-4">
      <div>
        <router-link :to="{ name: listRouteName }" class="text-sm font-medium text-gray-400 hover:text-gray-600">
          ← Back to {{ postType === 'blog' ? 'blogs' : 'articles' }}
        </router-link>
        <h1 class="mt-2 font-display text-2xl font-bold tracking-tight text-gray-900">
          {{ docId ? 'Edit' : 'New' }} {{ postType === 'blog' ? 'blog post' : 'article' }}
        </h1>
      </div>
      <div class="flex items-center gap-3">
        <span v-if="autoSaveState === 'saving'" class="text-xs text-gray-400">Saving draft…</span>
        <span v-else-if="autoSaveState === 'saved'" class="text-xs text-cyan-600">Draft saved</span>
        <button
          type="button"
    ...
    <div v-else class="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-[2fr,1fr]">
      <div class="space-y-6">
        ...
        <div class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-soft">
          ...
          <p class="mt-1 text-xs text-gray-400">Optional — one per line. Shown as a highlighted summary box (magazine-style pages only).</p>
          <textarea
        ...
        <div class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-soft">
          <div class="flex items-center justify-between">
            ...
            <button type="button" class="text-xs font-medium text-blue-600 hover:text-blue-700" @click="addFaq">
              + Add question
            </button>
          </div>
          <p class="mt-1 text-xs text-gray-400">Optional — renders as an accordion on the post and adds FAQ structured data.</p>

          <div v-if="form.faqs.length" class="mt-4 space-y-4">
            <div v-for="(faq, i) in form.faqs" :key="i" class="rounded-xl border border-slate-100 p-4">
              <div class="flex items-start gap-2">
                ...
                <button type="button" class="mt-1 text-xs font-medium text-red-500 hover:text-red-600" @click="removeFaq(i)">
                  Remove
                </button>
              </div>
            </div>
      ...
      <div class="space-y-6">
        ...
        <div class="space-y-4 rounded-2xl border border-slate-200/80 bg-white p-5 shadow-soft">
          ...
          <div>
            ...
            <input v-model="form.author" type="text" placeholder="Lumen Ledger Team" class="mt-1.5 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-blue-400" />
          </div>
          <div>
            <label class="text-xs font-medium text-gray-500">Tags (comma separated)</label>
            <input v-model="form.tags" type="text" placeholder="wallets, security" class="mt-1.5 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-blue-400" />
          </div>
          <div class="flex items-center justify-between text-sm text-gray-500">
            <span>Reading time</span>
            <span class="font-medium text-gray-900">{{ readingTime }} min</span>
          </div>
          <div v-if="docId" class="flex items-center justify-between text-sm text-gray-500">
            <span>Views</span>
            <span class="font-medium text-gray-900">{{ views.toLocaleString() }}</span>
          </div>

          <label class="flex items-center justify-between text-sm text-gray-700">
            Featured
            <input v-model="form.featured" type="checkbox" class="h-4 w-4 rounded border-slate-300 text-blue-600 focus:ring-blue-400" />
          </label>
          <label class="flex items-center justify-between text-sm text-gray-700">
            Published
            <input v-model="form.published" type="checkbox" class="h-4 w-4 rounded border-slate-300 text-blue-600 focus:ring-blue-400" />
          </label>
        </div>
        ...
        <div class="rounded-2xl border border-slate-200/80 bg-white shadow-soft">
          ...
          >
            ...
            <span class="text-gray-400 transition-transform duration-200" :class="seoOpen && 'rotate-180'">⌄</span>
          </button>

          <div v-show="seoOpen" class="space-y-4 border-t border-slate-100 p-5 pt-4">
            <div>
            ...
            <div>
              <label class="text-xs font-medium text-gray-500">SEO description</label>
              <textarea
                v-model="form.seoDescription"
                rows="2"
                :placeholder="form.description || 'Falls back to short description'"
                class="mt-1.5 w-full resize-none rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-blue-400"
              />
            </div>
            <div>
              <label class="text-xs font-medium text-gray-500">Focus keyword</label>
              <input
            ...
            <div>
              <label class="text-xs font-medium text-gray-500">Canonical URL</label>
              <input
            ...
            <div>
              ...
              <p class="mb-1.5 text-xs text-gray-400">Falls back to the cover image if left empty.</p>
    ...
    >
      <div v-if="previewOpen" class="fixed inset-0 z-[200] overflow-y-auto bg-white">
        <div class="sticky top-0 z-10 flex items-center justify-between border-b border-slate-100 bg-white/90 px-6 py-4 backdrop-blur">
          ...
          <button class="rounded-xl border border-slate-200 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-slate-50" @click="previewOpen = false">
            Close
          </button>
        </div>
        <article class="mx-auto max-w-3xl px-6 py-12">
          <p class="text-sm font-semibold text-blue-600">{{ form.category || 'Uncategorized' }}</p>
