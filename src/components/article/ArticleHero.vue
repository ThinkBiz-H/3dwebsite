...
const difficultyClass = computed(() => DIFFICULTY_STYLES[props.post.difficulty] || 'bg-slate-100 text-gray-500')
...
onMounted(async () => {
  ...
  revealHeading(titleEl.value, { type: 'words', trigger: titleEl.value, start: 'top 98%', stagger: 0.035, delay: 0.15 })
  tl.to(metaEl.value, { opacity: 1, y: 0, duration: 0.55, ease: 'power3.out' }, '-=0.15')
  if (imageEl.value) tl.to(imageEl.value, { opacity: 1, scale: 1, duration: 1.15, ease: 'power4.out' }, '-=0.25')
})
</script>
...
<template>
  <header class="bg-white pt-32 sm:pt-40">
    <div class="mx-auto max-w-3xl px-6 text-center lg:px-10">
      <div ref="introEl">
        <nav aria-label="Breadcrumb" class="flex flex-wrap items-center justify-center gap-2 text-xs font-medium text-gray-400">
          <router-link :to="{ name: 'home' }" class="transition-colors hover:text-gray-700">Home</router-link>
          <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="2" class="h-3 w-3 text-gray-300"><path d="m8 4 6 6-6 6" /></svg>
          <router-link :to="basePath" class="transition-colors hover:text-gray-700">{{ listLabel }}</router-link>
          <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="2" class="h-3 w-3 text-gray-300"><path d="m8 4 6 6-6 6" /></svg>
          <span class="max-w-[12rem] truncate text-gray-600 sm:max-w-xs">{{ post.title }}</span>
        </nav>
      ...
      <div ref="metaEl" class="mt-7 flex flex-wrap items-center justify-center gap-x-5 gap-y-3 text-sm text-gray-500">
        <div class="flex items-center gap-2.5">
          <AuthorAvatar :name="post.author || 'Lumen Ledger Team'" size="sm" />
          <span class="font-medium text-gray-800">{{ post.author || 'Lumen Ledger Team' }}</span>
        </div>
        <span class="h-1 w-1 rounded-full bg-gray-300" />
        <span class="flex items-center gap-1.5">
          <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.6" class="h-4 w-4 text-gray-400">
        ...
        <template v-if="post.views">
          ...
          <span class="flex items-center gap-1.5">
            <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.6" class="h-4 w-4 text-gray-400">
              <path d="M2 10s2.7-5 8-5 8 5 8 5-2.7 5-8 5-8-5-8-5Z" /><circle cx="10" cy="10" r="2" />
            </svg>
