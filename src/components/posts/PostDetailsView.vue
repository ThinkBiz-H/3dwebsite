...
watch(post, async (value) => {
  ...
  if (isMagazine.value) {
    ...
    tl.to(coverEl.value, { clipPath: 'inset(0% 0% 0% 0%)', scale: 1, duration: 1.3, ease: 'power4.out' }, 0)
      .to(headerEl.value, { opacity: 1, y: 0, duration: 0.8, ease: 'power3.out' }, '-=0.7')
    gsap.set(bodyEl.value, { opacity: 0, y: 16 })
    tl.to(bodyEl.value, { opacity: 1, y: 0, duration: 0.6, ease: 'power3.out' }, '-=0.3')
  } else {
    gsap.set([headerEl.value, coverEl.value], { opacity: 0, y: 24 })
    gsap.set(bodyEl.value, { opacity: 0, y: 16 })

    const tl = gsap.timeline({ delay: 0.05 })
    tl.to(headerEl.value, { opacity: 1, y: 0, duration: 0.7, ease: 'power3.out' })
      .to(coverEl.value, { opacity: 1, y: 0, duration: 0.8, ease: 'power3.out' }, '-=0.45')
      .to(bodyEl.value, { opacity: 1, y: 0, duration: 0.6, ease: 'power3.out' }, '-=0.4')
  }

  if (asideEl.value) {
    revealUp(asideEl.value.children, { trigger: asideEl.value, start: 'top 90%', y: 20, stagger: 0.1 })
  }
  if (extrasEl.value) {
    revealUp(extrasEl.value.children, { trigger: extrasEl.value, start: 'top 88%', y: 24, stagger: 0.12 })
  }
})
...
useSeoMeta(() => {
  ...
  return {
    ...
    jsonLd: articlePageSchema({
      ...
      breadcrumbItems: [
        { label: props.listRouteName === 'blog' ? 'Blog' : 'Articles', path: props.basePath },
        { label: p.title },
      ],
    }),
  }
...
<template>
  ...
  <div v-if="loading" class="px-6 pt-40 text-center text-sm text-gray-400">Loading…</div>

  <div v-else-if="notFound || error" class="mx-auto max-w-lg px-6 pt-40 pb-24 text-center">
    <h1 class="font-display text-3xl font-bold text-gray-900">{{ error ? 'Something went wrong' : 'Post not found' }}</h1>
    <p class="mt-3 text-gray-500">{{ error || 'This post may have been unpublished or moved.' }}</p>
  ...
  <article v-else ref="articleEl" class="pb-24">
    <div class="px-6 pt-28 lg:px-10" :class="isMagazine ? '' : 'pt-36'">
      <div class="mx-auto max-w-3xl">
        <Breadcrumbs
          :items="[{ label: listRouteName === 'blog' ? 'Blog' : 'Articles', to: basePath }, { label: post.title }]"
        />
      </div>
    </div>

    <!-- Magazine variant: full-bleed cover-behind-title hero -->
    <header v-if="isMagazine" class="relative isolate mt-6 min-h-[78vh] overflow-hidden bg-gray-950">
      <div class="absolute inset-0">
        ...
        />
        <div class="absolute inset-0 bg-gradient-to-t from-gray-950 via-gray-950/55 to-gray-950/15" />
      </div>

      <div ref="headerEl" class="relative flex min-h-[78vh] flex-col justify-end px-6 pb-14 lg:px-10">
        <div class="mx-auto w-full max-w-5xl">
          <Badge v-if="post.category" color="white" variant="soft">{{ post.category }}</Badge>
          <h1 class="mt-6 font-display text-4xl font-bold leading-[1.05] tracking-tight text-white sm:text-6xl lg:text-7xl">
            {{ post.title }}
          </h1>
          <div class="mt-8 flex flex-wrap items-center gap-4 text-sm text-white/70">
            ...
            <span class="h-1 w-1 rounded-full bg-white/40" />
            <div class="[&_a]:border-white/20 [&_a]:text-white/80 [&_a]:hover:border-white/40 [&_a]:hover:text-white [&_button]:border-white/20 [&_button]:text-white/80 [&_button]:hover:border-white/40 [&_button]:hover:text-white">
    ...
    <template v-else>
      <header ref="headerEl" class="mx-auto mt-8 max-w-3xl px-6 text-center lg:px-10">
        <span class="text-sm font-semibold text-blue-600">{{ post.category }}</span>
        <h1 class="mt-3 font-display text-3xl font-bold leading-[1.1] tracking-tight text-gray-900 sm:text-4xl lg:text-5xl">
          {{ post.title }}
        </h1>
        <div class="mt-6 flex flex-wrap items-center justify-center gap-3 text-sm text-gray-400">
          <Avatar :name="post.author || 'Lumen Ledger'" size="sm" />
    ...
    <div class="px-6 lg:px-10" :class="isMagazine ? 'mt-14' : 'mt-14'">
      <div ref="bodyEl" class="mx-auto grid max-w-5xl grid-cols-1 gap-12 lg:grid-cols-[1fr,220px]">
        <div class="min-w-0 lg:order-1">
          <div v-if="isMagazine" class="mb-10 grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div
            ...
            :class="isMagazine ? 'prose-lg first-letter:text-7xl' : 'first-letter:text-6xl'"
            v-html="contentHtml"
          />
          ...
          <div class="mt-8 border-t border-slate-100 pt-8">
            <p class="mb-3 text-sm font-medium text-gray-500">Share this post</p>
            <ShareButtons :title="post.title" :url="shareUrl" />
          </div>
          ...
          <nav class="mt-10 grid grid-cols-1 gap-4 border-t border-slate-100 pt-8 sm:grid-cols-2">
            <router-link
              v-if="prevPost"
            ...
            >
              <div class="min-w-0">
                <p class="flex items-center justify-end gap-1 text-xs text-gray-400">
                  Next
    ...
    <div ref="extrasEl">
      <div v-if="post.faqs?.length" class="mx-auto mt-16 max-w-5xl px-6 lg:px-10">
