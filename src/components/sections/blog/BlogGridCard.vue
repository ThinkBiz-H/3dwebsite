...
<template>
  ...
  >
    ...
    <div class="flex flex-1 flex-col p-6">
      <h3 class="text-lg font-semibold leading-snug text-gray-900 transition-colors duration-[350ms] group-hover:text-blue-600">
        {{ post.title }}
      </h3>
      ...
      >
        <span class="h-2 w-2 shrink-0 rounded-full" :style="{ backgroundColor: categorySwatch.fg }" />
        {{ post.category }}
      </span>
      ...
      <div class="mt-auto flex items-center gap-2 pt-5 text-sm text-gray-500">
        ...
        <span class="font-medium text-gray-700">{{ post.author || 'Lumen Ledger Team' }}</span>
        <span>•</span>
