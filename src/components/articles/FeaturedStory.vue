...
onMounted(() => {
  if (imgEl.value) revealMask(imgEl.value, { trigger: imgEl.value, start: 'top 80%' })
...
<template>
  ...
  >
    <div class="order-2 lg:order-1">
      ...
      <h2 class="mt-5 font-display text-3xl font-bold leading-[1.1] tracking-tight text-gray-900 transition-colors group-hover:text-blue-600 sm:text-4xl lg:text-5xl">
        {{ post.title }}
      </h2>
      <p class="mt-5 max-w-lg text-base leading-relaxed text-gray-500">{{ post.description }}</p>
      ...
      <span ref="ctaEl" class="relative mt-8 inline-flex w-fit items-center gap-2 rounded-full bg-blue-600 px-6 py-3 text-sm font-medium text-white shadow-lg shadow-blue-600/20 transition-colors group-hover:bg-blue-700">
    ...
    <div class="order-1 aspect-[4/3] overflow-hidden rounded-3xl bg-slate-100 shadow-lift lg:order-2">
      <img
